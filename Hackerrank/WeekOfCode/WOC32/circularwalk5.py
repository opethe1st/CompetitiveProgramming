...
def circularWalk(n, s, t, r_0, g, seed, p):
    ...
    while queue:
        ...
        for d in xrange(-R[u],R[u]+1):
            if dp[(u+d)%n]==float('inf'): # I need a measure of distance between two points simply abs(s-t) and (n-abs(s-t))%n
                dp[(u+d)%n]=dp[u]+1
...
n, s, t = 10000000,0,500000#raw_input().strip().split(' ')
